...
import { setPhoneOtpModal } from 'app/redux/actions/ModalActions';
...
const MobileVerificationModal = ({ individualProperty }) => {
    ...
    const open = useSelector((state) => state.modal.openPhoneOtp);
    ...
    const validationSchema = Yup.object().shape({
        mobileOtp: Yup.number().typeError('OTP should be a number').required('Please enter OTP')
    });
            ...
            listing_contact_details: [
                {
                    listing_contact_id: individualProperty
                        ?.listing_details
                        ?.contact_info
                        ?.filter((contact) => contact.contact_type === 2)?.[0].contact_id?.toString(),
                    otp: parseInt(values.mobileOtp)
                }
            ]
        }))
    ...
    const handleClose = () => {
        dispatch(setPhoneOtpModal(false));
                    ...
                    >
                        Enter OTP sent to your phone number 
                         <br />{individualProperty?.listing_details?.contact_info
                          ?.filter((contact) => contact.contact_type === 2)?.[0]?.contact_info}
                    </Typography>
                    ...
                    <Formik
                        ...
                        {({
                            ...
                            <Form onSubmit={handleSubmit}>
                                ...
                                <Grid marginY={2} container justifyContent='center'>
